...
import { requestBlogChanges } from "@/services/admin.service";
...
export async function POST(req: NextRequest, { params }: Props) {
  try {
    ...
    const blog = await requestBlogChanges(id, user.userId, parsed.data.message);
    if (!blog) return NextResponse.json(apiError("Blog not found"), { status: 404 });
    return NextResponse.json(apiSuccess(blog, "Changes requested"));
  } catch (err) {
